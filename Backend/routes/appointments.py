...
from db_com.appointments import  appointmentsComunicationDB 
...
com_db = appointmentsComunicationDB()
#citas
@router.get("/", response_model=List[Appointments]) 
async def get_all_appointments():
    try: 
        return com_db.get_list_of_appointments()
...
async def create_appointements(appointment: Appointments):
    try: 
        return com_db.create_appointment(appointment) 
    except Exception as e:
...
async def get_appointment(id: str):
    try:
        appointment = com_db.get_appointment_by_id(id) 
...
async def delete_appointment(id: str):
    try:
        com_db.delete_appointment_by_id(id) 
